...
export function studentPaymentReceiptPdf(receipt: StudentPaymentReceiptData) {
  ...
  pdf.text("MONEY RECEIPT", x + w / 2, y + 82, {
    align: "center",
    color: teal,
    font: "bold",
    size: 10,
  ...
  drawField(pdf, x + 24, y + 130, "RECEIPT NO", receipt.payment.receiptNo)
  drawField(pdf, right - 24, y + 130, "DATE", formatDate(receipt.payment.paymentDate), "right")

  pdf.line(x, y + 154, right, y + 154, slate200)
  drawField(pdf, x + 24, y + 184, "RECEIVED FROM", receipt.student.name)
  drawField(
    pdf,
    x + 235,
    y + 184,
  ...
  drawField(pdf, x + 24, y + 226, "BILLING MONTH", formatMonth(receipt.ledger.billingMonth))
  drawField(
    pdf,
    x + 235,
    y + 226,
    "BATCH",
    receipt.batchNames.length ? receipt.batchNames.join(", ") : "-"
  )
  ...
  drawAmountRow(pdf, tableX, tableY, tableW, "Payment Method", formatMethod(receipt.payment.method))
  drawAmountRow(
    pdf,
    tableX,
    tableY + 29,
    ...
    formatTaka(receipt.ledger.totalExpectedAmount)
  )
  drawAmountRow(
    pdf,
    tableX,
    ...
    tableW,
    "Paid Amount",
    formatTaka(receipt.payment.amount),
    "positive"
  )
  drawAmountRow(
    pdf,
    ...
    formatTaka(receipt.ledger.dueAmountAfterPayment),
    "danger"
  )
...
function drawAmountRow(
  ...
  label: string,
  value: string,
  tone: "danger" | "default" | "positive" = "default"
) {
  if (tone === "danger") {
    pdf.rect(x + 1, y + 1, width - 2, 28, red50)
  }
  if (y > receiptY + 274) {
    pdf.line(x, y, x + width, y, slate200)
  }
...
class PdfCanvas {
  ...
  rect(x: number, y: number, width: number, height: number, fill: Color, stroke?: Color) {
    ...
    this.commands.push(
      `${x} ${this.flipY(y + height)} ${width} ${height} re ${stroke ? "B" : "f"}`
    )
  }
  ...
  text(value: string, x: number, y: number, options: TextOptions = {}) {
    ...
    const offset =
      options.align === "right" ? textWidth : options.align === "center" ? textWidth / 2 : 0
    ...
    this.commands.push(
      [
        "BT",
        `/${font} ${size} Tf`,
        `${(x - offset).toFixed(2)} ${this.flipY(y).toFixed(2)} Td`,
...
function centerInfo(receipt: StudentPaymentReceiptData) {
  return receipt.tenant.contactPhone
    ? `${receipt.tenant.address || "-"} | Phone: ${receipt.tenant.contactPhone}`
